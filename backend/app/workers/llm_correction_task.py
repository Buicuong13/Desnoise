...
def llm_correct_page_task(self, page_id: str, user_role: str, provider: str | None = None) -> int:
    logger.info(
        "llm_correct_page_task started for %s (role=%s, provider=%s)",
        page_id,
        user_role,
        provider,
    )
    return llm_correct_page(page_id, user_role, provider)
